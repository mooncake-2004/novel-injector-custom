...
export function createStoryController(deps = {}) {
    ...
    function niPlotActionHtml(plot, type, index = 0) {
        const nodeId = niEnsurePlotNodeId(plot, type, index);
        return `<div class="ni-plot-node-actions">
          <button type="button" class="ni-plot-copy-btn" data-plot-type="${type}" data-node-id="${niEscAttr(nodeId)}" title="复制剧情正文"><i class="ti ti-copy"></i>复制</button>
          <button type="button" class="ni-plot-ai-rewrite-btn" data-plot-type="${type}" data-node-id="${niEscAttr(nodeId)}" title="使用 AI 修改剧情正文"><i class="ti ti-wand"></i>AI 修改</button>
        </div>`;
    }
            ...
            return `<div class="ni-tl-item${isPivot ? ' ni-tl-pivot' : ''}" id="${nodeId}">
              ...
              <div class="ni-tl-content">
                ...
                <div class="ni-tl-body">
                  ...
                  ${niPlotActionHtml(node, node._type, node._originalIdx ?? ni)}
                  ${subNotesHtml}
                  ${subLinksHtml}
                  ${metaHtml}
                </div>
        ...
        el.innerHTML = items.map((it, i) => {
            ...
            return `<div class="ni-plot-item" id="${id}" draggable="true" data-plot-type="${containerId}" data-plot-idx="${origIdx}" data-plot-pos="${i}" data-node-id="${niEscAttr(nodeId)}">
              ...
              <div class="ni-plot-body">
                ...
                ${niPlotActionHtml(it, it._type || (label === '支线' ? 'sub' : label === '转折' ? 'pivot' : 'main'), origIdx)}
                ${subNotesHtml}
                ${branchHtml}
    ...
    function niBuildStageFolderTimeline(stageIdx, nodes) {
        const typeMeta = { main: { label: '主线', cls: 'ni-bp' }, sub: { label: '支线', cls: 'ni-bt' }, pivot: { label: '转折', cls: 'ni-bc' } };
        const merged = [
            ...(nodes.main || []).map(p => ({ ...p, _type: 'main' })),
            ...(nodes.sub || []).map(p => ({ ...p, _type: 'sub' })),
            ...(nodes.pivot || []).map(p => ({ ...p, _type: 'pivot' })),
        ].sort(niComparePlotOrder);
        if (!merged.length) return '<div class="ni-empty ni-stage-folder-empty">本阶段暂无剧情节点</div>';
        return `<div class="ni-timeline ni-stage-folder-timeline">${merged.map((node, index) => {
            const meta = typeMeta[node._type] || typeMeta.main;
            const id = `ni-sft-${stageIdx}-${index}`;
            const metaHtml = (node.time || node.location) ? `<div class="ni-tl-meta">${node.time ? `<span class="ni-pmeta"><i class="ti ti-clock"></i>${niEscHtml(node.time)}</span>` : ''}${node.location ? `<span class="ni-pmeta"><i class="ti ti-map-pin"></i>${niEscHtml(node.location)}</span>` : ''}</div>` : '';
            return `<div class="ni-tl-item${node._type === 'pivot' ? ' ni-tl-pivot' : ''}" id="${id}"><div class="ni-tl-spine"><div class="ni-tl-dot${node._type === 'pivot' ? ' ni-tl-dot-pivot' : ''}"></div><div class="ni-tl-line"></div></div><div class="ni-tl-content"><div class="ni-tl-head" data-tl-id="${id}"><span class="ni-badge ${meta.cls}">${meta.label}</span><span class="ni-plot-name">${niEscHtml(node.title || '（未命名）')}</span><i class="ti ti-chevron-down ni-plot-chev"></i></div><div class="ni-tl-body"><p class="ni-plot-txt">${niEscHtml(node.body || '')}</p>${niPlotActionHtml(node, node._type, index)}${metaHtml}</div></div></div>`;
        }).join('')}</div>`;
    }
    ...
    function buildStages() {
        ...
        for (let i = 1; i <= n; i++) {
            ...
            item.innerHTML = `
              ...
              <button type="button" class="ni-stage-folder-head" data-stage-folder="${i}" aria-expanded="false">
                <span><i class="ti ti-folder"></i>阶段剧情</span><span class="ni-stage-folder-count">${nodes.main.length + nodes.sub.length + nodes.pivot.length} 个节点 <i class="ti ti-chevron-down"></i></span>
              </button>
              <div class="ni-stage-folder-body" id="ni-stage-folder-${i}" style="display:none">${niBuildStageFolderTimeline(i, nodes)}</div>
    `;
    ...
    async function niAutoStageByPivot({ confirmMap = false, silent = false, incremental = false } = {}) {
        if (!canUseDerivedModules(S)) { if (!silent) alert('请先完成至少一个分段并停止清洗，再进行阶段划分'); return false; }
        const main = S.plots.main || [];
        const pivot = S.plots.pivot || [];
        if (!main.length) { if (!silent) alert('请先完成清洗，生成剧情节点后再划分'); return false; }
        ...
        if (incremental) {
            // 续跑清洗只处理尚未分配的新节点：旧阶段和旧节点保持原样。
            const assignedStageIndexes = allNodes
                .map(node => Number(node.plot?.stageIdx))
                .filter(stageIdx => Number.isInteger(stageIdx) && stageIdx > 0);
            const latestStageIdx = Math.max(1, Number(S.stageMapN) || 0, ...assignedStageIndexes);

            for (let stageIdx = 1; stageIdx <= latestStageIdx; stageIdx++) {
                _stageSlots[stageIdx] = {
                    label: S.stageTitles?.[stageIdx] || `阶段 ${stageIdx}`,
                    assignedChunks: new Set(),
                };
            }
            _slotCounter = latestStageIdx;

            allNodes.forEach(node => {
                const stageIdx = Number(node.plot?.stageIdx);
                if (Number.isInteger(stageIdx) && stageIdx > 0 && _stageSlots[stageIdx]) {
                    _stageSlots[stageIdx].assignedChunks.add(node.ci);
                }
            });

            const unassigned = allNodes.filter(node => {
                const stageIdx = Number(node.plot?.stageIdx);
                return !Number.isInteger(stageIdx) || stageIdx <= 0;
            });
            const latestStageHasPivot = allNodes.some(node =>
                node.isPivot && Number(node.plot?.stageIdx) === latestStageIdx
            );
            let targetStageIdx = latestStageHasPivot ? latestStageIdx + 1 : latestStageIdx;

            for (const node of unassigned) {
                if (!_stageSlots[targetStageIdx]) {
                    _stageSlots[targetStageIdx] = {
                        label: `阶段 ${targetStageIdx}`,
                        assignedChunks: new Set(),
                    };
                    _slotCounter = Math.max(_slotCounter, targetStageIdx);
                }
                _stageSlots[targetStageIdx].assignedChunks.add(node.ci);
                // 关键转折归入当前阶段并封闭它，后续新节点进入下一阶段。
                if (node.isPivot) targetStageIdx += 1;
            }
        } else {
            // 手动“按转折点自动划分”仍执行全量重划。
            ...
            for (const node of allNodes) {
                ...
                if (node.isPivot) flushStage();
            }
            flushStage();
        }
    
        niRenderStageSlots();
        if (confirmMap) niConfirmStageMap();
