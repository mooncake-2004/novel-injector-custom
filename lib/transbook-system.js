...
export function createTransbookController(deps = {}) {
    ...
    async function niTbToggleCheck(idx) {
        ...
        const wasCurrentNode = node.id === S.tbCurNodeId || node.legacyId === S.tbCurNodeId;
        ...
        // 归档当前节点后，自动把正式进度移动到下一个未归档节点；允许跨阶段。
        let autoAdvancedNode = null;
        if (newDone && wasCurrentNode) {
            const freshNodes = niGetTbNodes();
            const archivedIdx = freshNodes.findIndex(item => item.id === node.id || item.legacyId === node.id);
            const nextIdx = freshNodes.findIndex((item, i) => i > archivedIdx && !item.done && !item.locked);
            if (nextIdx >= 0) {
                niTbSetCurrentIdx(nextIdx, freshNodes);
                autoAdvancedNode = freshNodes[nextIdx];
                niTbBuildTrack();
                niTbSyncMeta(freshNodes);
                niTbRefreshNodePanel(freshNodes);
                niTbRebuildStageList();
            }
        }
        ...
        // 节点完成后：若未暂停，向自动选中的下一节点发送推进提示词。
        if (newDone && !S.tbPaused) {
            if (autoAdvancedNode) {
                niTbWriteAdvancePrompt(node, autoAdvancedNode);
            } else {
