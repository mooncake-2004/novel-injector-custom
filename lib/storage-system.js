...
const NI_PRIVATE_EXPORT_SETTING_KEYS = new Set([
    ...
    'plotApiKey', 'plotApiUrl', 'plotApiModel',
]);
...
export function createStorageController(deps = {}) {
    ...
    async function niSaveNovelSnapshot(name, { notifyErrors = true, throwOnError = false } = {}) {
        if (!name) return;
        const cfg = extension_settings[EXT_NAME];
    ...
    async function niUpdateNovelSnapshot(idx, { confirmUpdate = true, notify = true, throwOnError = false } = {}) {
        const cfg = extension_settings[EXT_NAME];
