...
export function createCleaningController({
    ...
    async function niStartClean(options = {}) {
        ...
        renderPlots();
        renderCharacters();
        if (S.cleanDone && !hasError && !S.stopClean) {
            try { await onCleanCompleted(); }
            catch (error) { console.warn('[NI] 清洗完成后的自动阶段划分失败:', error); }
        }
        buildStages();
    ...
    async function niRunSingleChunk(i) {
        ...
        if (S.cleanDone) {
            try { await onCleanCompleted(); }
            catch (error) { console.warn('[NI] 单段清洗完成后的自动阶段划分失败:', error); }
        }
        niSyncCleanButtonState();
    }
